...
//to get order detailed data
export const POST = withUserAuth(async (req: NextRequest) => {
  try {
    const token = await getToken();
    const url = new URL(req.url);
    const strefid = url.searchParams.get("strefid");
    const reftag = url.searchParams.get("reftag");
    const acname = (req as any)?.user?.acname;

    if(!acname || !strefid || !reftag) {
      return NextResponse.json(
        { message: "Invalid request parameters" },
        { status: 400 }
      );
    ...
    const apiUrl = new URL("/orderbook/", process.env.APIV_URL);
    apiUrl.searchParams.set("reftag", reftag);
